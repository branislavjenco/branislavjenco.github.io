...
head_html = env.get_template("head.html").render()
...
for filename in posts_to_publish:
    ...
    if os.path.isfile(file_path):
        with open(file_path) as f:
            ...
            single_post_rendered = single_post_template.render(title=title, date=date, body=post_html, head=head_html, footer=footer_html)
...
with open(f"about.md") as md_file:
    ...
    with open(f"{build_folder}/about.html", 'w') as html_file:
        result = about_template.render(about=about_html, head=head_html, footer=footer_html)
        html_file.write(result)

with open(f"{build_folder}/index.html", 'w') as f:
    result = index_template.render(posts=posts_html, head=head_html, footer=footer_html)
    f.write(result)
