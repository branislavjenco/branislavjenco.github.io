...
The docker image uses the [jemalloc](https://github.com/jemalloc/jemalloc) allocator, but as will be shown, has nothing to do with it. This was happening only in certain environments - not in the Azure Kubernetes Service (AKS) and not on all hosts running regular `docker`, however it seemed to apply to specifically some hosts running older versions of `docker`. The problem turned out to be the following:
...
Some relevant discussions on github:
